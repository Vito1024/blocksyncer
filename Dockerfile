...
# 构建二进制文件
RUN GOPROXY=https://goproxy.cn,direct GOOS=${GO_OS} GOARCH=${GO_ARCH} go build -v -o blocksyncer -ldflags '-s -w' cmd/main.go
...
COPY --chown=sato --from=build /usr/local/build/blocksyncer /data/blocksyncer
...
# 启动命令
CMD ["./blocksyncer", "-config", "config.yaml"]